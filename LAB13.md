# Reflected Cross-Site Scripting (XSS) - Academy Parameter Injection

## Summary

I identified a Reflected Cross-Site Scripting (XSS) vulnerability in the `academy` parameter of the application.

The application accepts user-controlled input through the `academy` GET parameter and reflects it back to the page without proper sanitization or output encoding. By injecting a malicious JavaScript payload, arbitrary code execution is achieved when the victim visits the crafted URL.
...
```text
https://kzlabs.in/13.php?academy=javascript:alert(1)
```
...
2. Replace the value of the `academy` parameter with the following payload:
...
The application accepts the value of the `academy` parameter as a navigation target or page reference without validating the URI scheme.
...
The `javascript:` URI scheme allows arbitrary JavaScript code to be executed when the browser navigates to or processes the value. This confirms that arbitrary JavaScript execution is possible through the `academy` parameter.

![Uploading Screenshot 2026-08-14 195708.png…]()
...
- **Output Encoding**: Use `htmlspecialchars()` (or an equivalent output-encoding function) before rendering user-controlled data in HTML to convert special characters to their HTML entities.
  ```php
  echo htmlspecialchars($_GET['academy'], ENT_QUOTES, 'UTF-8');
  ```
...
- **URI Scheme Validation**: When accepting URLs or navigation targets, validate the URI scheme and only allow safe protocols such as `http://`, `https://`, and `mailto:`.
  ...
  $parsed_url = parse_url($_GET['academy']);
