# Reflected Cross-Site Scripting (XSS) - Script Tag Injection

## Summary

I identified a Reflected Cross-Site Scripting (XSS) vulnerability in the `q` parameter of the application.

The application accepts user-controlled input through the `q` GET parameter and reflects it back to the page without proper sanitization or output encoding. By injecting a `<SCRIPT>` tag with malicious JavaScript, arbitrary code execution is achieved when the victim visits the crafted URL.
...
```text
https://kzlabs.in/?php?q='>anil<SCRIPT>alert(1)</SCRIPT>
```
...
3. Visit the crafted URL:
   ...
   ```text
   https://kzlabs.in/?php?q=%27%3Eanil%3CSCRIPT%3Ealert%281%29%3C/SCRIPT%3E
   ```

4. Observe that the browser executes the injected `<SCRIPT>` tag and displays an `alert(1)` popup.
...
URL-encoded version:
```text
%27%3Eanil%3CSCRIPT%3Ealert%281%29%3C/SCRIPT%3E
```
...
This confirms that arbitrary JavaScript execution is possible through the `q` parameter.

<img width="1651" height="849" alt="Screenshot 2026-08-14 185356" src="https://github.com/user-attachments/assets/31e3dd4a-ebbc-4888-85de-672196723cfd" />
